...
def clean_strict(text):
    """Removes EVERYTHING that crashes PDF: Emojis, link symbols, and non-standard bullets."""
    if not text: return ""
...
def create_pdf(title, itinerary):
    ...
    # Force output to bytes to fix the 'bytearray' error
...
::placeholder {{ color: #555555 !important; opacity: 1 !important; font-weight: 500; }}
...
with c_greet: st.markdown(f"## Hello, {st.session_state.first_name}!")
with c_out: 
    if st.button("Logout"):
...
if st.session_state.user_role == "Admin":
    ...
    # 1. PLACEHOLDERS (Hard-coded as requested)
    ...
    if st.session_state.itinerary:
        ...
        with btn1:
            st.download_button("📊 Excel", pd.DataFrame(st.session_state.itinerary).to_csv(index=False).encode('utf-8'), f"{fn}.csv")
        with btn2:
            st.download_button("📝 Word", create_word(it_name, st.session_state.itinerary), f"{fn}.docx")
        with btn3:
            try:
                pdf_bytes = create_pdf(it_name, st.session_state.itinerary)
                st.download_button("📄 PDF", pdf_bytes, f"{fn}.pdf", mime="application/pdf")
            except Exception as e:
                st.error(f"Error: {e}. Please remove special symbols from your text.")
